...
-- 5.  Выведите имена, дату рождения студентов ,
--     которые родились с 198011 по 20041212, и названиегруппы
...
-- age(date '2025-01-16', s.date_of_birth)
select s.first_name, s.last_name, s.email, 2025 - extract(year from s.date_of_birth) as age, g.group_name
from students s
...
select *
from courses c
         join groups g on g.id = c.groups_id
where g.group_name like 'Java 13';
--
...
-- 10. Отсортируйте имена студентов группы по убыванию, где айди группы
-- равна 4 и выведите айдистудента, имя, пол и название группы
...
select *
from courses c
         join groups g on c.groups_id = g.id
         join students s on g.id = s.group_id
where c.id = 4;
-- 4. Вывести имя, почту, специализацию ментора и название курса где курс
-- айди равен 2
...
-- 6. Сгруппируйте и посчитайте менторов в каждом курсе и выведите только те
-- курсы, где в курсебольше 2 менторов

select c.course_name , count(m.id)
from  courses c join mentors m on c.id = m.course_id
group by  c.course_name  having count(m.id) >= 2;

-- 7. Вывести название, дату и полное имя ментора, все курсы которые
-- начинаются с 202011 по 202333
select c.id, c.start_date, c.course_name, m.first_name from courses c join  mentors m on c.id = m.course_id
   where c.start_date between  '2020-01-01' and '2023-03-03';
